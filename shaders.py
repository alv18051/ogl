...
void main()
{
    ...
    fragColor = texture(texture0, UVs) * vec4(1.0, 1.0 - sin(time * 2), 1.0 - sin(time * 2), 1.0) * intensity;
}
...
{
    float intensity = dot(norms, normalize(pointLight - pos));
    fragColor = texture(texture0, UVs) * vec4(0.0, 1.0, 0.0, 1.0) * intensity;
}
